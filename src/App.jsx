...
import {
	initialState, initialAppVars, voidFuncs
}
	from "./calculator/Constants";
import {
	keysReducer, exprsTokensReducer
}
	from "./calculator/ReducerFns";


const App = () => {
	const [keyState, dispatchKeyState] = useReducer(keysReducer, initialState);

	const [exprTokenState, dispatchExprTokenState] = useReducer(exprsTokensReducer, {
		exprs: {
			left: [],
			right: []
		},
	...
	const activateError = (error = "Network Error") => {
	...
	const fetchApiExtend = ({
		...
		if (endpoint === "main") {
			...
			if (lastCommaIndex < 1 && expression === "") return;
			if (lastCommaIndex === -1) {
				fetchApiMain(data);
				return;
			} else {
